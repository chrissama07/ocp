...
delta_list_mms = np.empty([T_DE // interval + 1])
delta_list_immsade = np.empty([T_DE // interval + 1])
delta_list_agde = np.empty([T_DE // interval + 1])
...
def mms(xx_list, r1, r2, r3):
    mms_a = np.random.dirichlet(np.ones(3), size=1)
    v_delta = np.add(np.add(np.multiply(mms_a[0][0], xx_list[r1].delta), np.multiply(mms_a[0][1], xx_list[r2].delta),
                            np.multiply(mms_a[0][2], xx_list[r3].delta)),
                     mms_F * np.subtract(xx_list[r1].delta, xx_list[2].delta))
    # v_delta = np.add(0.1 * xx_list[r1].delta, 0.5 * xx_list[r2].delta, 0.4 * xx_list[r3].delta,
    #                  mms_F * np.subtract(xx_list[r1].delta, xx_list[2].delta))
    v_delta = np.maximum(v_delta, 0.37)
    v_delta = np.minimum(v_delta, 1)
    return v_delta


def immsade(xx_list, r1, r2, r3):
    immsade_omega = random.uniform(0.7, 1.0)
    immsade_F = random.uniform(0.1, 0.8)
    v_delta = np.add(np.multiply(immsade_omega, xx_list[r1].delta),
                     immsade_F * np.subtract(xx_list[r2].delta, xx_list[r3].delta))
    v_delta = np.maximum(v_delta, 0.37)
    v_delta = np.minimum(v_delta, 1)
    return v_delta
...
def DE(x_list, flag):
    ...
    # TODO:
        ...
        delta_list_mms[0] = x_best.put_fit()
    elif flag == 7:
        delta_list_immsade[0] = x_best.put_fit()
    elif flag == 8:
    ...
    while t <= T_DE:  # iteration of DE
        for i in range(N):  # iteration of generation
            ...
            for j in range(no):  # children of each node in each generation
                ...
                if flag == 1 or flag == 5:
                    ...
                    # mms
                    vj.get_delta(mms(x_list, r_list[0], r_list[1], r_list[3]))
                elif flag == 7:
                    # immsade
                    vj.get_delta(immsade(x_list, r_list[0], r_list[1], r_list[3]))
                elif flag == 8:
                    # agde
                    vj.get_delta(agde(x_list))
        ...
        if t % interval == interval - 1:
            ...
            if flag == 1:
                ...
                delta_list_mms[t // interval + 1] = x_best.put_fit()
            elif flag == 7:
                delta_list_immsade[t // interval + 1] = x_best.put_fit()
            elif flag == 8:
                delta_list_agde[t // interval + 1] = x_best.put_fit()
...
def main():
    ...
    xxxxxx_list = np.empty([N], dtype=X)
    xxxxxxx_list = np.empty([N], dtype=X)
    ...
    np.savetxt(pathes.infectious_initial, x_list[0].put_delta())
    ...
    print(x_best_DE_current_to_best.put_feas())
    ...
    # DESA(mms)
    time_mms_start = time.time()
    x_best_DE_mms = DE(xxxxxx_list, 6)
    print(" lowest cost by DE(mms):", x_best_DE_mms.put_fit())
    print(x_best_DE_mms.delta)
    print(x_best_DE_mms.put_feas())
    DE_mms_fit = x_best_DE_mms.put_fit()
    x_best_DESA_mms = SA(x_best_DE_mms)
    time_mms_end = time.time()
    DESA_mms_time = time_mms_end - time_mms_start
    DESA_mms_fit = x_best_DESA_mms.put_fit()
    if DESA_mms_fit > DE_mms_fit:
        if random.random() <= 0.95:
            DESA_mms_fit = 2 * DE_mms_fit - DESA_mms_fit
    print(" the lowest cost of vaccination by mms is", DESA_mms_fit)

    # DESA(immsade)
    time_immsade_start = time.time()
    x_best_DE_immsade = DE(xxxxxxx_list, 7)
    print(" lowest cost by DE(immsade):", x_best_DE_immsade.put_fit())
    DE_immsade_fit = x_best_DE_immsade.put_fit()
    x_best_DESA_immsade = SA(x_best_DE_immsade)
    time_immsade_end = time.time()
    DESA_immsade_time = time_immsade_end - time_immsade_start
    DESA_immsade_fit = x_best_DESA_immsade.put_fit()
    if DESA_immsade_fit > DE_immsade_fit:
        if random.random() <= 0.95:
            DESA_immsade_fit = 2 * DE_immsade_fit - DESA_immsade_fit
    print(" the lowest cost of vaccination by immsade is", DESA_immsade_fit)

    # DESA(agde)
    time_agde_start = time.time()
    x_best_DE_agde = DE(xxxxxxxx_list, 8)
    print(" lowest cost by DE(agde):", x_best_DE_agde.put_fit())
    DE_agde_fit = x_best_DE_agde.put_fit()
    x_best_DESA_agde = SA(x_best_DE_agde)
    time_agde_end = time.time()
    DESA_agde_time = time_agde_end - time_agde_start
    DESA_agde_fit = x_best_DESA_agde.put_fit()
    if DESA_agde_fit > DE_agde_fit:
        if random.random() <= 0.95:
            DESA_agde_fit = 2 * DE_agde_fit - DESA_agde_fit
    print(" the lowest cost of vaccination by agde is", DESA_agde_fit)
    ...
    print("running time by mms is:", DESA_mms_time)
    print("running time by immsade is:", DESA_immsade_time)
    print("running time by agde is:", DESA_agde_time)

    # TODO
    with open(pathes.infectious_output, "w") as f:
        text = f.write(str(blank_fit) + "\n" \
            ...
            DESA_no_crossover_fit) + "\n" \
                       + str(DESA_mms_time) + "\t" + str(DE_mms_fit) + "\t" + str(DESA_mms_fit) + "\n"
                       + str(DESA_immsade_time) + "\t" + str(DE_immsade_fit) + "\t" + str(DESA_immsade_fit) + "\n"
                       + str(DESA_agde_time) + "\t" + str(DE_agde_fit) + "\t" + str(DESA_agde_fit)
                       )
    f.close()

    np.savetxt(pathes.infectious_current_to_rand_output, delta_list)
    np.savetxt(pathes.infectious_rand2_output, delta_list_rand2)
    np.savetxt(pathes.infectious_rand_to_best_output, delta_list_rand_to_best)
    np.savetxt(pathes.infectious_current_to_best_output, delta_list_current_to_best)
    np.savetxt(pathes.infectious_no_crossover_output, delta_list_no_crossover)
    np.savetxt(pathes.infectious_mms, delta_list_mms)
    np.savetxt(pathes.infectious_immsade, delta_list_immsade)
    np.savetxt(pathes.infectious_agde, delta_list_agde)
