...
import pathes
from networks.infectious import data_infectious

A = data_infectious.A

N = data_infectious.N  # number of nodes

E = data_infectious.E  # number of edges

Cn = data_infectious.Cn  # cost of each vaccine

gamma = data_infectious.gamma  # curing rate

Rou = data_infectious.Rou  # vaccination rate
...
def AGDE(x_list):
    ...
    delta_list_agde[0] = x_best.put_fit()
...
def main():
    print(A[..., 2])
    # initialize X
    xxxxxxxx_list = np.empty([N], dtype=X)
    ...
    x_best_DE_agde = AGDE(xxxxxxxx_list)
    ...
    np.savetxt(pathes.infectious_agde_algorithm, delta_list_agde)
