...
import pathes
from networks.bay import data_bay

A = data_bay.A

N = data_bay.N  # number of nodes

E = data_bay.E  # number of edges

Cn = data_bay.Cn  # cost of each vaccine

gamma = data_bay.gamma  # curing rate
...
interval = 5
delta_list = cp.empty(T_DE // interval + 1)
delta_list_rand2 = cp.empty(T_DE // interval + 1)
delta_list_rand_to_best = cp.empty(T_DE // interval + 1)
delta_list_current_to_best = cp.empty(T_DE // interval + 1)
delta_list_no_crossover = cp.empty(T_DE // interval + 1)
...
class X:
    ...
    def fitness_value(self):
        ...
        fitness = cp.multiply(self.delta, Cn).sum()
    ...
    def get_fit(self):
        self.fit = cp.multiply(self.delta, Cn).sum()
        # for i in range(N):
...
def current_to_rand(xx_list, r1, r2, r3, i):
    ...
    """
    v_delta = cp.add(xx_list[i].delta, k * cp.subtract(xx_list[r1].delta, xx_list[i].delta),
                     F * cp.subtract(xx_list[r2].delta, xx_list[r3].delta))

    return v_delta


def rand_2(xx_list, r1, r2, r3, r4, r5):
    v_delta = cp.add(xx_list[r1].delta, k * cp.subtract(xx_list[r2].delta, xx_list[r3].delta),
                     F * cp.subtract(xx_list[r4].delta, xx_list[r5].delta))
    return v_delta


def rand_to_best(xx_list, r1, r2, r3, best):
    v_delta = cp.add(xx_list[r1].delta, k * cp.subtract(xx_list[best].delta, xx_list[r1].delta),
                     F * cp.subtract(xx_list[r2].delta, xx_list[r3].delta))
    return v_delta


def current_to_best(xx_list, r1, r2, r3, best, i):
    v_delta = cp.add(xx_list[r1].delta, k * cp.subtract(xx_list[best].delta, xx_list[i].delta),
                     F * cp.subtract(xx_list[r2].delta, xx_list[r3].delta))
    return v_delta
...
def crossover_SBX(xi, vj):
    ...
    c1_delta = 0.5 * cp.add((1 + beta) * xi_delta, (1 - beta) * vj_delta)  # crossover
    c2_delta = 0.5 * cp.add((1 - beta) * xi_delta, (1 + beta) * vj_delta)
...
def main():
    print("start")
    # initialize X
    x_list = cp.empty(N, dtype=X)  # list of vectors
    xx_list = cp.empty(N, dtype=X)  # list of vectors
    xxx_list = cp.empty(N, dtype=X)  # list of vectors
    xxxx_list = cp.empty(N, dtype=X)  # list of vectors
    xxxxx_list = cp.empty(N, dtype=X)  # list of vectors
    blank_fit = 0
    for i in range(N):  # initialize x_list
        p = X()
        r_delta = cp.array([random.uniform(0, 1) for _ in range(N)])
        p.get_delta(r_delta)
        p.get_fit()
        p.get_feas()
        x_list[i] = p
    ...
    cp.savetxt(pathes.bay_initial, x_list[0].put_delta())
    ...
    # TODO
    with open(pathes.bay_output, "w") as f:
    ...
    cp.savetxt(pathes.bay_current_to_rand_output, delta_list)
    cp.savetxt(pathes.bay_rand2_output, delta_list_rand2)
    cp.savetxt(pathes.bay_rand_to_best_output, delta_list_rand_to_best)
    cp.savetxt(pathes.bay_current_to_best_output, delta_list_current_to_best)
    cp.savetxt(pathes.bay_no_crossover_output, delta_list_no_crossover)
