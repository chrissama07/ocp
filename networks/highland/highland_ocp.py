...
delta_list = np.empty([T_DE])
delta_list_rand2 = np.empty([T_DE])
delta_list_rand_to_best = np.empty([T_DE])
delta_list_current_to_best = np.empty([T_DE])
delta_list_no_crossover = np.empty([T_DE])
...
# def select_best_iteration(x_list):
#     min_fit = 0
#     for i in range(len(x_list)):
#         if x_list[i].put_feas() <= 0:
#             min_fit = x_list[i].put_fit()
#             break
#     for i in range(len(x_list)):
#         if min_fit > x_list[i].put_fit() & x_list[i].feas() <= 0:
#             min_fit = x_list[i].put_fit()
#     return min_fit
...
def DE(x_list, flag):
    ...
    while t <= T_DE:  # iteration of DE
        ...
        # TODO
        if flag == 1:
            delta_list[t - 1] = x_best.put_fit()
        elif flag == 2:
            delta_list_rand2[t - 1] = x_best.put_fit()
        elif flag == 3:
            delta_list_rand_to_best[t - 1] = x_best.put_fit()
        elif flag == 4:
            delta_list_current_to_best[t - 1] = x_best.put_fit()
        elif flag == 5:
            delta_list_no_crossover[t - 1] = x_best.put_fit()
