...
with open(pathes.macaques_output) as f:
    lines = f.readlines()
lines.pop(0)
...
npsai = np.loadtxt(pathes.macaques_SA_output)
nptsi = np.loadtxt(pathes.macaques_TS_output)
npagdei = np.loadtxt(pathes.macaques_agde_algorithm)
npgski = np.loadtxt(pathes.macaques_GSK)
...
plt.ylim(300 * 100, 460 * 100)
plt.xlim(-1, 5)
plt.savefig(draw_pathes.macaques_pictures_bar_2)
