...
with open(pathes.highschool_output) as f:
    lines = f.readlines()
lines.pop(0)
...
npsai = np.loadtxt(pathes.highschool_SA_output)
nptsi = np.loadtxt(pathes.highschool_TS_output)
npagdei = np.loadtxt(pathes.highschool_agde_algorithm)
npgski = np.loadtxt(pathes.highschool_GSK)
...
plt.title('Algorithm Comparison of Interaction', size=20)
...
plt.ylim(380 * 100, 490 * 100)
plt.xlim(-1, 5)
plt.savefig(draw_pathes.highschool_pictures_bar_2)
