...
with open(pathes.zachary_output) as f:
    lines = f.readlines()
lines.pop(0)
...
npsai = np.loadtxt(pathes.zachary_SA_output)
nptsi = np.loadtxt(pathes.zachary_TS_output)
npagdei = np.loadtxt(pathes.zachary_agde_algorithm)
npgski = np.loadtxt(pathes.zachary_GSK)
