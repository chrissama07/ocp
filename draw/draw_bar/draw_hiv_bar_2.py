...
with open(pathes.hiv_output) as f:
    lines = f.readlines()
lines.pop(0)
...
npsai = np.loadtxt(pathes.hiv_SA_output)
nptsi = np.loadtxt(pathes.hiv_TS_output)
npagdei = np.loadtxt(pathes.hiv_agde_algorithm)
npgski = np.loadtxt(pathes.hiv_GSK)
...
plt.title('Algorithm Comparison of Hiv', size=20)
...
plt.ylim(220, 260)
plt.xlim(-1, 5)
plt.savefig(draw_pathes.hiv_pictures_bar_2)
