...
y5 = np.loadtxt(draw_pathes.league_mms_output)
y6 = np.loadtxt(draw_pathes.league_immsade_output)
y7 = np.loadtxt(draw_pathes.league_agde_output)
...
# plt.plot(x1, y5, label='mms', linewidth=1, color='tan', marker='H')
# plt.plot(x1, y6, label='immsade', linewidth=1, color='crimson', marker='+')
# plt.plot(x1, y5, label='agde-mutation', linewidth=1, color='dimgray', marker='<')
# plt.xticks([i for i in range(5, 50, 5)])
# # plt.legend()  # 给label用的
# plt.grid(alpha=0.4, linestyle='--')
# plt.xlabel('Iteration', size=15)
...
#
# plt.xticks([i for i in range(0, 50, 5)])
# ax2.set_ylim(475, 490)
# ax3.set_ylim(390, 411)
...
plt.savefig(draw_pathes.league_pictures_linechart_1)
plt.show()
