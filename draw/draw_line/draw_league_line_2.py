...
# y_blank=np.loadtxt(draw_pathes.league_blank_output)
# y0=np.linspace(y_blank,y_blank,21)
y = np.loadtxt(draw_pathes.league_rand_to_best_output)
y1 = np.loadtxt(draw_pathes.league_rand2_output)
y2 = np.loadtxt(draw_pathes.league_current_to_rand_output)
y3 = np.loadtxt(draw_pathes.league_no_crossover_output)
y4 = np.loadtxt(draw_pathes.league_current_to_best_output)
y5 = np.loadtxt(pathes.league_TS_output)
y6 = np.loadtxt(pathes.league_SA_output)
y7 = np.loadtxt(draw_pathes.league_mms_output)
y8 = np.loadtxt(draw_pathes.league_immsade_output)
y9 = np.loadtxt(draw_pathes.league_agde_output)
y10 = np.loadtxt(pathes.league_agde_algorithm)
y11 = np.loadtxt(pathes.league_GSK)
# for i in range(20):
...
plt.title('Sexual Escorts', size=18)
...
#plt.figure(figsize=(8, 6))
plt.title('Sexual Escorts Subgraph', size=18)
# plt.plot(x, y0, label='blank', linewidth=1, color='black')
...
plt.xlabel('Iteration', size=15)
plt.ylabel('Lowest Cost of Vaccination', size=15)
plt.savefig(draw_pathes.league_pictures_linechart_2)
plt.show()
