...
y = np.loadtxt(draw_pathes.highschool_rand_to_best_output)
y1 = np.loadtxt(draw_pathes.highschool_rand2_output)
y2 = np.loadtxt(draw_pathes.highschool_current_to_rand_output)
y3 = np.loadtxt(draw_pathes.highschool_no_crossover_output)
y4 = np.loadtxt(draw_pathes.highschool_current_to_best_output)
y5 = np.loadtxt(pathes.highschool_TS_output)
y6 = np.loadtxt(pathes.highschool_SA_output)
# for i in range(20):
...
plt.plot(x, y6, label='SA', linewidth=1, color='y', marker='.')
...
#plt.figure(figsize=(8, 6))
plt.title('highschool_subgraph', size=18)
# plt.plot(x, y0, label='blank', linewidth=1, color='black')
...
plt.xlabel('Iteration', size=15)
plt.ylabel('Objective function value', size=15)
plt.savefig(draw_pathes.highschool_pictures_linechart_1)
plt.show()
