...
# y_blank=np.loadtxt(draw_pathes.jazz_blank_output)
...
y5 = np.loadtxt(pathes.jazz_TS_output)
y6 = np.loadtxt(pathes.jazz_SA_output)
y7 = np.loadtxt(draw_pathes.jazz_mms_output)
y8 = np.loadtxt(draw_pathes.jazz_immsade_output)
y9 = np.loadtxt(draw_pathes.jazz_agde_output)
# y10 = np.loadtxt(pathes.jazz_agde_algorithm)
y11 = np.loadtxt(pathes.jazz_GSK)
# for i in range(20):
...
plt.xlabel('Iteration', size=15)
plt.ylabel('Lowest Cost of Vaccination', size=15)
plt.savefig(draw_pathes.jazz_pictures_linechart_2)
plt.show()
